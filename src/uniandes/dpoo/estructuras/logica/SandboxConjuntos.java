...
public class SandboxConjuntos
{
    ...
    {
    	Collection<String> siguientes = new ArrayList<String>();

        for (String actual : arbolCadenas)
        {
            if (actual.compareTo(cadena) >= 0)
            {
    ...
    {
    	List<String> copia = getCadenasComoLista();

        arbolCadenas.clear();

        for (int i = 0; i < copia.size(); i++)
        {
            arbolCadenas.add(copia.get(i).toUpperCase());
        }
    }
    ...
    {
    	 TreeSet<String> invertido = new TreeSet<String>(arbolCadenas.descendingSet());
