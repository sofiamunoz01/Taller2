...
public class SandboxListas
{
    ...
    public void organizarEnteros( )
    {
    	 for (int i = 0; i < listaEnteros.size(); i++)
    	    {
    	        for (int x = i + 1; x < listaEnteros.size(); x++)
    	        {
    	            if (listaEnteros.get(i) < listaEnteros.get(x))
    	            {
    	                int temp = listaEnteros.get(i);

    	                listaEnteros.set(i, listaEnteros.get(x));
    	                listaEnteros.set(x, temp);
    	            }
    	        }
    	    }
    }
    ...
    public void organizarCadenas( )
    {
        ...
        for (int i = 0; i < listaCadenas.size(); i++)
        {
            for (int x = i + 1; x < listaCadenas.size(); x++)
            {
                if (listaCadenas.get(i).compareTo(listaCadenas.get(x)) > 0)
                {
                    String temp = listaCadenas.get(i);

                    listaCadenas.set(i, listaCadenas.get(x));
                    listaCadenas.set(x, temp);
                }
            }
        }
    }
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (int i = 0; i < listaEnteros.size(); i++)
        {
            if (contarApariciones(listaEnteros.get(i)) > 1)
            {
                boolean contado = false;

                for (int x = 0; x < i; x++)
                {
                    if (listaEnteros.get(x) == listaEnteros.get(i))
                    {
                        contado = true;
                    }
                }
